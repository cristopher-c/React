...
export const ScrambleWords = () => {
  const [arrayWords, setArrayWords] = useState(shuffleArray(GAME_WORDS));

  const [currentWord, setCurrentWord] = useState(arrayWords[0]);
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if(guess === currentWord){
      ...
      const newArrayWords = arrayWords.slice(1);
  ...
  const handleSkip = () => {
    console.log(`Length before skip: ${arrayWords.length}`);
    console.log(`Current word ${currentWord}\n\n`);
    const newArrayWords = arrayWords.slice(1);
    setArrayWords(newArrayWords);
    console.log(`Length after skip word: ${newArrayWords.length}`);
    setSkipCounter(skipCounter + 1);
    setCurrentWord(newArrayWords[0]);
    setScrambledWord(scrambleWord(newArrayWords[0]));
    console.log(`Palabra saltada ${currentWord}\n\n`);
  };
  ...
  const handlePlayAgain = () => {
    ...
    setSkipCounter(0);
    setScrambledWord(scrambleWord(newArrayWords[0]));
    console.log(newArrayWords === arrayWords)
  };
